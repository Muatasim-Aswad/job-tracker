...
The dashboard refreshes its review queues while it remains visible. Questions handled by an active Match leave the actionable queue, while unmatched, disabled, retired, or conflicting Questions remain available for review. Choice-option Matches must be complete and one-to-one. Large option-binding sets start collapsed to keep the drawer usable, and each selector lists its current choice first. A supported Question that appears later in the same application step is observed after its identity stays stable across two scans.
