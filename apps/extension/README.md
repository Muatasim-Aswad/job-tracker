...
The dedicated LinkedIn entry runs in the top document and matching same-origin frames. Within each document it follows open shadow roots, including LinkedIn's interop shell, so the same scanner observes an Easy Apply modal whether LinkedIn mounts it in light DOM or Shadow DOM. It waits for lazy fields to settle and re-establishes its safe baseline on forward, backward, and repeated steps. Supported controls are ordinary text and textarea inputs, classified numeric inputs, selects, and ordinary single-choice radio groups whose request-local options can be bound to server-owned option IDs.
...
Supported controls that appear after a step's initial render enter resolution only after their identity remains stable across two scans. LinkedIn numeric text inputs default to integer unless the control supplies an explicit decimal signal; the write guard still rejects decimal values for an integer field. Typeaheads, malformed or unidentified radio groups, consent checkboxes, other unclassified utility controls, and repeatable profile sections stay local and manual and are omitted from resolution requests. Selects and radio groups with up to 512 options retain their complete option identity and can be remembered and mapped normally; select examples include phone-country-code lists. A larger choice control stays manual because the API validates a resolution request as one bounded model, so sending it would reject the whole scan and leave every sibling field unchecked. Résumé selectors, file inputs, and LinkedIn's top-choice preference are ignored entirely: they produce no request, marker, or status entry. LinkedIn's preselected follow-company preference is cleared once when the control appears, remains outside form-fill knowledge, and can still be selected deliberately afterward. The scanner never clicks or invokes Next, Continue, Review, Submit, consent, profile, file, résumé, follow-company, or top-choice actions, and it never navigates or submits an application.
