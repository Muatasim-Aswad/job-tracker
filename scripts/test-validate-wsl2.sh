...
printf '%s\n' "$missing_checksums" | rg -Fq -- '--checksums is required for a real validation run' || {
  printf '%s\n' 'test-validate-wsl2: real runs must reject a missing checksum file.' >&2
  exit 1
}

for required in '--artifact' '--extension' '--checksums is required for a real validation run' "\$(uname -m)\" == x86_64" "grep -qi 'wsl2' /proc/sys/kernel/osrelease" 'verify_supplied_hash "$ARTIFACT"' 'verify_supplied_hash "$EXTENSION"' 'runtime filename does not match extracted VERSION' 'extension filename does not match extracted runtime VERSION' 'backup' 'restore' 'Windows-host localhost' 'Candidate, unsupported'; do
  rg -Fq -- "$required" "$ROOT/scripts/validate-wsl2.sh" "$ROOT/docs/WSL2.md" "$ROOT/docs/evidence/wsl2-template.md" || {
